...
def init(engine, base=Base, should_create=False, should_drop=False, authority=None):
    ...
    _maybe_create_world_organization(engine, authority)
...
def _maybe_create_world_organization(engine, authority):
    from h import models
    session = Session(bind=engine)
    world_org = session.query(models.Organization).filter_by(pubid='__world__').one_or_none()
    if world_org is None:
        world_org = models.Organization(name=u'Hypothesis',
                                        authority=authority,
                                        pubid = '__world__',
                                        )
        with open(r'h/static/images/icons/logo.svg', 'r') as h_logo:
            # Skip the first line in the file as it's the xml version info.
            h_logo.readline()
            world_org.logo = h_logo.read()
        session.add(world_org)
